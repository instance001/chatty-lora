...
There is now also a first scaffolded `AI Toolkit / Wan 2.2 TI2V 5B` lane. This is the app's first planned Diffusers-style Wan route rather than a Musubi safetensors route. Right now Chatty-lora can detect the local bundle shape, prepare a dataset JSONL, and generate a first-pass handoff folder, but this lane should still be treated as groundwork rather than a verified end-to-end pipeline until the local AI Toolkit recipe is proven.

For that lane, Chatty-lora expects a separate local AI Toolkit runtime checkout under `runtime/ai-toolkit/` or `runtime/ai_toolkit/`. The app treats that as a shared trainer bucket for future Diffusers-style routes too, not just this one Wan experiment. The current upstream AI Toolkit project explicitly spans Flux, SDXL, SD 1.5, Wan video models including `Wan2.2-TI2V-5B`, and even newer audio-capable families, so it is the natural place for future non-Musubi lanes to converge.
